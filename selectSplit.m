...
function cutfaces = selectSplit(data, node, alreadycutfaces, forbiddenfaces)
    ...
    if all(~alreadycutfaces)
        ...
        signature = accumarray(data.efdeg(node.v2E),1); if numel(signature) < 7; signature(7)=0; end;
        ...
        if sum(signature(6:end))==0
            ...
            if all(signature(3:5)==[0 0 12]')
                %% (0,0,12) = (0 2 8 1) +_6 (0 4 4 1)
                vdegs = data.efdeg(node.v2E);
                edegs = sort(vdegs(node.e),2);
                
                % get a starting val 5 v and a val 5 v adjacent to that
                % first val 5 v.
                startv = find(vdegs==5,1);
                adjv = setdiff(node.e(sum(node.e==startv,2)~=0,:),startv);
                secondv = adjv(find(vdegs(adjv)==5,1));
        ...
        elseif sum(signature(7:end))==0
            if all(signature(3:6)==[0 2 8 1]')
                %% (0 2 8 1) = (0 3 6) +_5 (0 3 6)
                vdegs = data.efdeg(node.v2E);
                edegs = sort(vdegs(node.e),2);
                tdegs = sort(vdegs(node.t),2);
                
                startt = find(all(tdegs==[4 5 5],2),1); % start with a 4-5-5 degree triangle.
                % get all triangles adj to start triangle.
                side1 = sum(ismember(node.t, node.t(startt,:)),2)~=0;
            ...
            if all(signature(3:6)==[0 4 4 1]')
                %% (0 4 4 1) = (0 4 4) +_5 (0 4 4)
                vdegs = data.efdeg(node.v2E);
                edegs = sort(vdegs(node.e),2);
                tdegs = sort(vdegs(node.t),2);
                
                % get triangles adajcent to a degree 4,5 edge that is
                % adjacent to the degree 6 vert.
                v6 = find(vdegs==6);
                v_adj_6 = unique(node.e(any(node.e==v6,2),:));
                e45_ondiam = find(all(ismember(node.e,v_adj_6),2) & all(edegs==[4 5],2),1);
                side1 = sum(ismember(node.t, node.e(e45_ondiam,:)),2)~=0;
